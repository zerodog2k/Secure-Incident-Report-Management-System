...
def initdb():
    ...
    if not admin:
        ...
        # Better: set DEFAULT_ADMIN_PASSWORD in your .env
        default_admin_pass = os.getenv("DEFAULT_ADMIN_PASSWORD", "Admin@12345")
        admin.set_password(default_admin_pass)
        db.session.add(admin)
        db.session.commit()
        print(f"Created default admin: admin@example.com / {default_admin_pass}")
    else:
...
def register_post():
    ...
    user = User(full_name=full_name, email=email, role="Reporter", is_active=True)
    user.set_password(password)

    db.session.add(user)
    db.session.commit()

    # Optional: auto-login after register (comment out if you prefer manual login)
    # login_user(user)
    audit("USER_REGISTER", details=f"Self-registered user {email}")
...
def forbidden(_):
    return (
        render_template(
            "base.html",
            content_title="403 Forbidden",
            content_body="You do not have permission to access this page."
        ),
        403,
    )
...
def not_found(_):
    return (
        render_template(
            ...
            content_body="The resource does not exist."
        ),
        404,
    )
